...
@pytest.fixture
def jobs_with_local_structured_log(initial_data):
    ...
    # substitute the log url with a local url
    job['job']['log_references'][0]['url'] = url
    job['job']['log_references'][0]['name'] = 'structured-raw'
    return [job]
...
def test_parse_structured_log(jm, initial_data, jobs_with_local_structured_log,
    ...
    jobs = jobs_with_local_structured_log
    for job in jobs:
    ...
    artifact = [x for x in job_artifacts if x['name'] == 'structured_faults']
...
def test_bug_suggestions_artifact(jm, initial_data, jobs_with_local_log,
    ...
    structured_log_artifact = [artifact for artifact in job_artifacts
                               if artifact["name"] == "log_summary"][0]
